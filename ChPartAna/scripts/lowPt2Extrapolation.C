...
void lowPt2Extrapolation(int iBin = 5 , double E = 0.9 ) {
  ...
  TH1* ptdata   = (TH1F*) data->Get("pt2/pt2_corrected");
  TH1* ptmccor  = (TH1F*) mc->Get("pt2/pt2_corrected");
  TH1* ptmcgen  = (TH1F*) mc->Get("pt2/pt2_MC_gen_afterCorrection");
  BinWidthDiv(ptdata);
  BinWidthDiv(ptmccor);
  BinWidthDiv(ptmcgen);
  ...
  TF1* FitPtData = new TF1("FitPtData","[0] * exp([1]*x+[2]*x*x+0.1396*x*x*x+[4]*x*x*x*x)" , 0.01 , .5 );
  TF1* FitPtMC   = new TF1("FitPtMC"  ,"[0] * exp([1]*x+[2]*x*x+0.1396*x*x*x+[4]*x*x*x*x)" , 0.01 , .5 );
  ...
  Double_t DataExtra = FitPtData->Integral(0.,.01 );
  Double_t MCExtra   = FitPtMC->Integral(0.,.01 );
